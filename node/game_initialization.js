...
var game_initialization = function(request){
    ...
    serverfile.app.io.route('gameSubmit', function(req) {
    	...
    	serverfile.connection.query('INSERT INTO game SET ?', game, function(err, result){
			...
			serverfile.connection.query('UPDATE user SET game_id = ? WHERE user_id = ?', [game_id, req.session.user_id], function(err, result){
				if (err) {
					console.error(err);
					return;
				}
		    	...
		    	serverfile.connection.query('INSERT INTO history SET ?', history, function(err, result){
					...
					serverfile.connection.query('INSERT INTO `game owner` (user_id, game_id) values (?, ?)', [req.session.user_id, game_id], function(err, result) {
						if (err) {
							console.error(err);
							return;
						}
