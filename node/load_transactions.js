...
var load_transactions = function(request){
	...
	serverfile.app.io.route('loadBids', function(req){
		serverfile.connection.query('SELECT `seller list`.seller_number, `buyer list`.buyer_number FROM bid INNER JOIN `buyer list` on bid.buyer_id = `buyer list`.buyer_id LEFT JOIN `seller list` on bid.seller_id = `seller list`.seller_id WHERE `buyer list`.game_id = ?', userGame, function(err, result) {
			if(err) {
				console.error(err);
				return;
			}
			...
			serverfile.connection.query('SELECT `buyer list`.buyer_number, user.teamname FROM `buyer list` INNER JOIN user on `buyer list`.user_id = user.user_id WHERE user.game_id = ?', userGame, function(err, result) {
				if(err) {
					console.error(err);
					return;
				}
	...
	serverfile.app.io.route('loadOffers', function(req) {
    	serverfile.connection.query('SELECT offers.quality_id, offers.price, `seller list`.seller_number, user.teamname FROM `seller list` INNER JOIN user on `seller list`.user_id = user.user_id INNER JOIN offers on offers.seller_id = `seller list`.seller_id WHERE `seller list`.game_id = ? ORDER BY `seller list`.seller_number ASC LIMIT 3;', userGame, function(err, result){
    		if (err) {
				console.error(err);
				return;
			}
			...
			for(var i = 0; i < result.length; i++){
				if(result[i]["seller_number"] == 1)
					offers.seller1 = {name: result[i]["teamname"], quality : result[i]["quality_id"], price : result[i]["price"]};
				else if(result[i]["seller_number"] == 2)
					offers.seller2 = {name: result[i]["teamname"], quality : result[i]["quality_id"], price : result[i]["price"]};
				else if(result[i]["seller_number"] == 3)
					offers.seller3 = {name: result[i]["teamname"], quality : result[i]["quality_id"], price : result[i]["price"]};
			}
