	...
	serverfile.connection.query('SELECT * FROM history WHERE game_id = ?', userGame, function(err, result){
		...
		serverfile.connection.query('SELECT `sale history`.history_id, `seller list`.seller_number, `sale history`.units_sold, `sale history`.price_sold, `sale history`.quality_id FROM `sale history` INNER JOIN `seller list` on `sale history`.seller_id = `seller list`.seller_id WHERE game_id = ? ORDER BY `sale history`.history_id ASC', userGame, function(err, rows){
			...
			serverfile.app.io.route('loadHistory', function(req){
				serverfile.connection.query('SELECT `buy history`.history_id, `buy history`.seller_number FROM `buy history` INNER JOIN `buyer list` ON `buy history`.buyer_id = `buyer list`.buyer_id WHERE `buyer list`.user_id = ? ORDER BY history_id ASC', req.session.user.user_id, function(err, rows){
					if(rows.length != 0){
						for(i = 0; i<result.length; i++){
							cur_history = result[i]["history_id"];
							for(n = 0; n<rows.length; n++){
					...
					serverfile.connection.query('SELECT `user`.teamname, `seller list`.seller_number FROM user INNER JOIN `seller list` on `user`.user_id = `seller list`.user_id WHERE `user`.game_id = ? ORDER BY seller_number ASC', userGame, function(err, rows){
						if (err) {
							console.error(err);
							return;
						}
