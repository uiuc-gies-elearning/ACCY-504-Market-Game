...
var game_room = function(request) {
  ...
  serverfile.app.io.route("loadGames", function(req) {
    serverfile.connection.query("SELECT game_id, game_name FROM game", function(
      err,
      result
    ) {
      if (err) {
  ...
  serverfile.app.io.route("selectGame", function(req) {
    ...
    serverfile.connection.query(
      "SELECT user_id FROM `game owner` WHERE game_id = ?",
      game_id,
      function(err, result) {
        if (err) {
          console.error(err);
        ...
        var isOwner = false;
        if (result[0]["user_id"] == user) isOwner = true;
        if (req.session.user.role_id == 3 && !isOwner)
          req.io.emit("joinFail", "notOwner");
        else {
          serverfile.connection.query(
            "SELECT user_id, role_id FROM user WHERE game_id = ?",
            game_id,
            function(err, result) {
              if (err) {
                console.error(err);
              ...
              if (result.length >= 8) req.io.emit("joinFail", "gameFull");
                ...
                var sellerCount = 0;
                for (var i = 0; i < result.length; i++) {
                  if (result[i]["role_id"] == 1) buyerCount++;
                  else if (result[i]["role_id"] == 2) sellerCount++;
                }
                  ...
                  serverfile.connection.query(
                    ...
                    function(err, result) {
                      ...
                      if (role == 1) {
                        ...
                        serverfile.connection.query(
                          "SELECT seller_number FROM `seller list` ORDER BY seller_id DESC LIMIT 1",
                          function(err, result) {
                            if (err) {
                              console.error(err);
                              return;
